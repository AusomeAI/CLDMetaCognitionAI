...
- `lucide-react` icons
- `three` / `@react-three/fiber` / `@react-three/drei` / `@react-three/postprocessing` for the
  optional 3D "Neural Constellation" graph view (bloom-glow nodes, animated energy-flow edges,
  starfield backdrop), lazy-loaded so it never costs 2D-only users a byte
